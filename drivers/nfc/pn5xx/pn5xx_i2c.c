...
static void pn54x_print_hex_data(char *data, size_t count)
{
	char buf_hex[MAX_BUFFER_SIZE];
	int buf_off = 0;
	int i;

	for (i = 0; i < count; i++) {
		if (i == count - 1)
			buf_off += snprintf(buf_hex + buf_off, MAX_BUFFER_SIZE - buf_off,
					"0x%02x", data[i]);
		else
			buf_off += snprintf(buf_hex + buf_off, MAX_BUFFER_SIZE - buf_off,
					"0x%02x ", data[i]);
	}
	pr_info("%s : count: %zu, [%s]\n", __func__, count, buf_hex);
}
		...
		size_t count, loff_t *offset)
{
	...
	if (DEBUG_I2C) {
		pr_info("%s: dump data\n", __func__);
		pn54x_print_hex_data(tmp, count);
	}
		...
		size_t count, loff_t *offset)
{
	...
	for (i = 0; i < retries; i++) {
		...
		if (ret == count)
			break;
		pr_warning("%s : retrying, current: %d, total: %d\n", __func__, i, retries);
		udelay(1000);
	}
	if (ret != count) {
...
static int pn54x_send_reset_cmd(struct pn54x_dev *dev)
{
	const char cmd[4] = {0x20, 0x00, 0x01, 0x00};
	...
	for (i = 0; i < retries; i++) {
		ret = i2c_master_send(dev->client, cmd, 4);
		if (ret == 4)
			break;
		else
			pr_warning("%s : retrying, current: %d, total: %d\n", __func__, i, retries);
	...
	if (ret < 0) {
		pr_err("%s : failed to send i2c message, ret: %d\n", __func__, ret);
		return ret;
	}
	udelay(1000);
	...
	if (!gpio_get_value(dev->irq_gpio)) {
		while (1) {
			...
			ret = wait_event_interruptible_timeout(dev->read_wq,
					!dev->irq_enabled, msecs_to_jiffies(100));
			pn54x_disable_irq(dev);
	...
	for (i = 0; i < retries; i++) {
		ret = i2c_master_recv(dev->client, data, 6);
		if (ret == 6)
			break;
		else
			pr_warning("%s : retrying, current: %d, total: %d\n", __func__, i, retries);
	...
	if (ret < 0) {
		pr_err("%s : failed to receive i2c message, ret: %d\n", __func__, ret);
		goto fail;
	}
	udelay(1000);
	mutex_unlock(&dev->read_mutex);

	pn54x_print_hex_data(data, 6);
...
#endif
{
	...
	pn544_enable(pn54x_dev, MODE_RUN);
	ret = pn54x_send_reset_cmd(pn54x_dev);
	if (ret < 0) {
		pr_err("%s : failed to send reset command\n", __func__);
		goto err_request_irq_failed;
	}
	pn544_disable(pn54x_dev);
