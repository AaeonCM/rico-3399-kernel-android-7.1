...
int xr_usb_serial_ctrl_msg(struct xr_usb_serial *xr_usb_serial, int request, int value,
							void *buf, int len)
{
				...
				const struct usb_device_id *id)
{
	...
	dev_info(&intf->dev, "USB_device_id idVendor:%04x, idProduct %04x\n",id->idVendor,id->idProduct);
...
skip_normal_probe:
	...
	if (!combined_interfaces && usb_interface_claimed(data_interface)) {
		struct usb_driver *driver = to_usb_driver(data_interface->dev.driver);

		/* valid in this context */
		dev_warn(&intf->dev, "The data interface isn't available\n");
		dev_warn(&intf->dev, "Unbind the driver '%s' from the interface\n",
				(&data_interface->dev)->driver->name);
		usb_driver_release_interface(driver, data_interface);
//		return -EBUSY;
	}
...
/* this init must be prior to the cdc_acm init */
/* or the /dev/ttyXRUSB will not be created */
fs_initcall(xr_usb_serial_init);
